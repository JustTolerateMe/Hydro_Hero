...
create table if not exists public.urine_logs (
  id bigint generated by default as identity primary key,
  user_id uuid references auth.users not null,
  color_scale int check (color_scale between 1 and 10),
...
-- Migration: Update urine_logs for 10-point scale (run if table already exists):
-- ALTER TABLE public.urine_logs DROP CONSTRAINT IF EXISTS urine_logs_color_scale_check;
-- ALTER TABLE public.urine_logs ADD CONSTRAINT urine_logs_color_scale_check CHECK (color_scale BETWEEN 1 AND 10);
